...
% mvc
mvcLeft      = 2.012;   % V — set per participant
mvcRight     = 2.389;   % V — set per participant
mvc_duration = 3;
...
%sombrero
use_constant_slope = true;   % true = derive ramps from slope (constant slopes); false = fixed durations
brim_height = 0.4; % fraction of task level (for mid plateau height)
brim_length = 5; % length of the mid plateau
brim_ramp_to_peak = 10; % length of ramp from mid to peak plateau (only needed when constant slope is false)
...
force_right = 142;
force_sum   = 143;
extra_channels = [69, 139, 145];
% channels:
% 65,66,67,68 are IMU, 69 is sync trigger for muovi 1, 70 is sample counter
% 135,136,137,138 are IMU, 139 is sync trigger for muovi 2, 140 is sample counter
% 141,142,143,144 are syncstation AUX, 145 is syncstation trigger, 146 is sample counter
...
% --- constant-slope ramp calc (for sombrero/half_sombrero) ---
if use_constant_slope
    ...
    brim_level  = task_level * brim_height;
    ramp1_steps = ramp_steps;              % old: fixed trap_ramp_s for both
...
function [task_force, task_emg] = run_multi_target(tcpSocket, ax, hl, hr, hs, ...
    ...
    trap_ramp_s, trap_hold_s, lead_in_s, emg_channels, n_emg, ConvFact, ...
    force_scale, colours , brim_height, brim_length, brim_ramp_to_peak, use_constant_slope)
