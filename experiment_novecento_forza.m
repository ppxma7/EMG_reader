...
%% FORCE INPUT & FORZA-B CONFIGURATION
use_forza_aux = true;          % true = Forza via AUX 1/2; false = Direct LOAD CELL 1/2
forza_gain_L  = 200;           % Selected Forza-b Gain: 100, 200, 500, or 1000
forza_gain_R  = 200;           % Selected Forza-b Gain: 100, 200, 500, or 1000
force_scale_L_direct = 0.0185; % arm rig  
force_scale_R_direct = 0.0185;

if use_forza_aux
    % AUX 1 and AUX 2 in rear 16-ch block
    force_ch_L = 1;  
    force_ch_R = 2;

    % Accounting for bypassing internal 205x gain AND adding Forza gain * 0.5 AUX gain
    force_scale_L = force_scale_L_direct * (410 / forza_gain_L);
    force_scale_R = force_scale_R_direct * (410 / forza_gain_R);
else
    % Dedicated LOAD CELL 1 and LOAD CELL 2 inputs
    force_ch_L = 5;  
    force_ch_R = 6;  
    
    force_scale_L = force_scale_L_direct;
    force_scale_R = force_scale_R_direct;
end
...
%   rows end-3 : end          -> accessory ch 1-4 (32-bit: count

% force_left  = nGrids*70 + 5;
% force_right = nGrids*70 + 6;

force_left  = nGrids*70 + force_ch_L;
force_right = nGrids*70 + force_ch_R;
