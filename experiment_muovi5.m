...
draw_counter = 0;
draw_every   = 1;   % render at ~20fps instead of 40fps
...
% show target
target_line = plot(ax, 1:N, NaN(1,N), 'r', 'LineWidth', 5);
...
t_render = tic;
render_interval = 0.075;  % render every 75ms
col = 1;
for k = 1:n_target
    ...
    while tcpSocket.BytesAvailable < bytesPerBlock, pause(0.001); end
    fprintf('%d bytes = %.0f ms worth\n', tcpSocket.BytesAvailable, tcpSocket.BytesAvailable/TotNumByte/sampFreq*1000);
